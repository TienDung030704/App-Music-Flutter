...
class PlayController {
    ...
    public function startPlaySession() {
        try {
            ...
            $sql = "INSERT INTO user_play_sessions (user_id, song_type, song_id, session_start) VALUES (?, ?, ?, NOW())";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$userId, $songType, $songId]);
    ...
    public function endPlaySession() {
        try {
            ...
            // Update play session
            $sql = "UPDATE user_play_sessions 
                    SET session_end = NOW(), play_duration = ?, counted_as_play = ? 
                    WHERE id = ?";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$playDuration, $countedAsPlay ? 1 : 0, $sessionId]);

            // Get session details to increment play count
            $sql = "SELECT song_type, song_id FROM user_play_sessions WHERE id = ?";
            $stmt = $this->db->prepare($sql);
