...
try {
    ...
    error_log("DEBUG Play routing - segments: " . implode(',', $segments) . " playIndex: $playIndex method: $method");
    if ($playIndex !== false) {
    ...
    if ($method === 'GET' && isset($segments[2]) && $segments[2] === 'songs') {
        $sdk = new NCT();
        $controller = new SongController($sdk);
        
        if (isset($segments[3]) && $segments[3] === 'search') {
            $controller->search($_GET['q'] ?? null);
            return;
        }
        
        if (isset($segments[3]) && $segments[3] === 'top') {
            $controller->getTop();
            return;
        }

        $songId = $segments[3] ?? null;
        if ($songId) {
            if (isset($segments[4]) && $segments[4] === 'lyric') {
    ...
    // API routes: /api/*
    if (isset($segments[2]) && $segments[2] === 'api') {
        // Test route
        if (isset($segments[3]) && $segments[3] === 'test') {
            Response::success(['message' => 'API routing works!']);
            return;
        }
        
        // Notification routes: /api/notifications/*
        if (isset($segments[3]) && $segments[3] === 'notifications') {
            $controller = new NotificationController();
            
            if ($method === 'POST') {
                if (isset($segments[4]) && $segments[4] === 'mark-read') {
                    // POST /api/notifications/mark-read - Mark notification as read
                    $controller->markAsRead();
                    return;
                }
                
                if (isset($segments[4]) && $segments[4] === 'mark-all-read') {
            ...
            if ($method === 'GET') {
                if (isset($segments[4]) && $segments[4] === 'unread-count') {
        ...
        if (isset($segments[3]) && $segments[3] === 'downloads') {
            ...
            if ($method === 'GET') {
                if (isset($segments[4]) && $segments[4] === 'check') {
            ...
            if ($method === 'DELETE' && isset($segments[4])) {
                ...
                $controller->removeDownload($segments[4]);
                return;
            }
        }
    }
